...
export interface Methods<TState, TPayload = undefined> {
  readonly [x: string]: Method<TState, TPayload>;
}
/**
 * @param  {TState} state
 * @param  {TPayload} payload
...
export interface Services<TState, TPayload = undefined> {
  readonly [x: string]: Service<TState, TPayload>;
}
 ...
 */
type WithPayload<T> = (
  payload: GetParams<T>[1]
) => { readonly payload: GetParams<T>[1]; type: string };
/**
 * @param  {string}} =>{type
 * @returns string
 */
...
type TAction<TA> = GetParams<TA>[1] extends undefined
  ? WithoutPayload
  : WithPayload<TA>;
